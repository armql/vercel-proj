...
export default function Tab({selectedTab, setSelectedTab}: {
    ...
    const {data: categories, isLoading, error} = useQuery('category_products', fetchCategories);
        ...
        <div className="flex w-full flex-col items-center justify-center py-0 sm:flex-row sm:py-6">
            {categories && categories.map((tab) => (
                <ul
                    ...
                    className={`grid w-full grid-cols-1 items-center justify-center gap-4 border-b-2 py-2 transition-all duration-300 ${selectedTab === tab.id ? "border-blue-500" : "border-blue-100"}`}
                >
