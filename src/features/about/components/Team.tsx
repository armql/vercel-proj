...
export default function Team() {
  ...
  const [teamData] = useState<TeamData[]>(team_data);
          ...
          <div className="flex flex-row">
            {team_data.map((tab) => (
              <ul
                ...
                className={`grid grid-cols-1 items-center justify-center gap-4 border-b-2 py-2 transition-all duration-300 ${
          ...
          <div className="team_cursor relative flex h-[350px] min-w-[375px] w-[375px] cursor-none flex-row gap-4 rounded-lg sm:p-4 md:w-[800px]">
            ...
            >
              <div className="flex h-full w-full flex-row gap-4">
                {teamData
                  .filter((data) => data.identifier === selected)
                  .map((data) => (
                              ...
                              <p
                                className={` ${
                                  response.personality
                                    ? "font-libre text-sm font-light tracking-wide text-zinc-300"
                                    : "font-mono text-xs uppercase tracking-tight text-white"
                                }`}
                              >
                                {response.role}
                              </p>
